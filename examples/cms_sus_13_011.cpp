...
double calculateChi2(std::vector<const RecJetFormat *> myjets)
{
  ...
  for ( int i=0; i<n_jets; ++i )
    ...
    {
      //  W
      TLorentzVector hadW = myjets[i]->momentum() + myjets[j]->momentum();
  ...
  for ( int b=0; b<n_jets; ++b )
  {
    ...
    for (unsigned int w = 0; w < v_i.size() ; ++w )
    {
      ...
      // Top Mass
      TLorentzVector hadT = (myjets[i]->momentum()*c1)+(myjets[j]->momentum()*c2)+myjets[b]->momentum();
